...
class reloc(_BaseByzantine): ## ortho to ps & proj start rand
    ...
    def omniscient_callback(self,benign_gradients):
        ...
        mu = torch.mean(stacked_gradients, 1).to(self.device) ## mean of the benign gradients ## reference point (global momentum)
        ...
        if self.first:
            ...
            self.first = False
            self.global_momentum = torch.zeros_like(mu).to(self.device) ## set the global momentum to zero
        else:
