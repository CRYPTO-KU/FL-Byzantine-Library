...
def googleAI_ERK(module, density, erk_power_scale: float = 1.0):
    ...
    for e, (name, weight) in enumerate(module.named_parameters()):
        ...
        if e == 0:
            exc_vec.append(1)
            excluded_.append(torch.ones_like(weight))
            total_excluded += (weight.numel())
            continue
        # Exclude bias
    ...
    for name, mask in masks.items():
        mask.data = (torch.rand_like(mask) < density_dict[name]).float()

    new_masks = []
    mask = list(masks.values())
    for layer in module.modules():
        if isinstance(layer,nn.Conv2d):
            #mask.insert(0,torch.ones_like(layer.weight))
            break
    #mask_flat = torch.cat([m.flatten() for m in mask])
    #print(mask_flat.sum(),mask_flat.mean())
    i = 0
    for excluded, _ in zip(excluded_, exc_vec):
        if excluded is not None:
            new_masks.append(excluded.flatten())
        else:
            new_masks.append(mask[i].flatten())
            i += 1
    new_mask_flatten = torch.cat([m.flatten() for m in new_masks])
    return new_mask_flatten
